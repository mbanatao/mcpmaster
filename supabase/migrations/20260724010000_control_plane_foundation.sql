...
create table public.credential_refs (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  installation_id uuid not null unique references public.connector_installations(id) on delete cascade,
...
create index workflow_steps_run_sequence_idx
  on public.workflow_steps(run_id, sequence);
create index workflow_steps_org_status_idx
  on public.workflow_steps(organization_id, status);
...
begin
  ...
  select approval.*,
         coalesce(step.risk, 'R1'::public.risk_class)
  into approval_row, step_risk
  from public.approvals approval
  left join public.workflow_steps step on step.id = approval.step_id
...
begin
  ...
  return private.append_audit_event(
    ...
    actor_user_id,
    event_type,
    coalesce(payload_redacted, '{}'::jsonb)
  );
end;
$$;
...
create trigger connector_installations_set_updated_at
  ...
  for each row execute function private.set_updated_at();
create trigger credential_refs_set_updated_at
...
create policy memberships_insert_admin
  ...
  with check (
    ...
    and (
      ...
      or role in ('operator'::public.member_role, 'member'::public.member_role, 'viewer'::public.member_role)
    )
  );
...
create policy memberships_update_admin
  ...
  with check (
    ...
    or (
      ...
      and role in ('operator'::public.member_role, 'member'::public.member_role, 'viewer'::public.member_role)
    )
  );
...
create policy workflow_runs_insert_operator
...
revoke execute on function private.append_audit_event(uuid, uuid, uuid, public.audit_actor_type, uuid, text, jsonb) from public, anon, authenticated, service_role;
...
revoke execute on function public.decide_approval(uuid, public.approval_decision, text) from public, anon, authenticated;
grant execute on function public.decide_approval(uuid, public.approval_decision, text) to authenticated;

revoke execute on function public.record_audit_event(uuid, text, public.audit_actor_type, jsonb, uuid, uuid, uuid) from public, anon, authenticated, service_role;
